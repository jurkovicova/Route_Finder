...
vector<tuple<double, double>> calculatePathCoordinates(Airport airport1, Airport airport2) {
    ...
    for(double i = 0; i < (numOfSteps + 1); i++) {
        ...
        intermediateLat = atan2(z, sqrt(pow(x, 2) + pow(y, 2))) * 180 / M_PI; // converting back to degrees
        intermediateLong = atan2(y, x) * 180 / M_PI;    // converting back to degrees
...
// print coordinates in a JSON format for further processing
...
void printLabels(string airport1, string airport2) {
    cout << "var labels = [";
    cout << "\"" << airport1 << "\"" << ",";
    cout << "\"" << airport2 << "\"]" << endl;
}
...
int main(int argc, char **argv) {
    ...
    for(Airport airport : loadedDatabase) { 
     ...
     if ((airport1.name.empty()) || (airport2.name.empty())) {
        ...
        printLabels(airport1.name, airport2.name);
     }
